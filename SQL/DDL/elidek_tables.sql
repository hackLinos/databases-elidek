...
-- DEFAULT CHARSET specifies the table character set, in this case UTF8. Note that, technically speaking, there is no such thing as a table character set. --
--Each text column has an associated character set, and it could be different for each column.  --
-- The table character set is a default: whenever you don't specify a character set for a column, the table character set applies. --

-- ENGINE specifies the storage engine to associate to the table. A storage engine is a special plugin (prostheto) that implements (efarmozo) all the functionalities (litourgikotita) related to a table: --
-- writing rows and indexes, reading rows and indexes, caches, repair, etc. --
-- It supports row-level locking, crash recovery and multi-version concurrency control. It is the only engine which provides foreign key referential integrity constraint. --
-- InnoDB is the default storage engine (unless you change the default_storage_engine variable) and it is the one that performs best in most situations. --
--  Also, it is the only transactional storage engine that is shipped with MySQL --
...
CREATE TABLE IF NOT EXISTS `elidek`.`ergo` (
  ...
  `ID_aksiologiti` INT NOT NULL,  --Prosoxi 2 foreign keys apaforeutai na exoun to idio onoma
